...
| Behavior Diff Gate (v2.3.18) — replays changed `bin/*.js` against harvested fixtures and reports behavior deltas. Invoked at release-readiness time (v2.3.19 check g), diffing against the last release tag — not on interactive edits, which the replay latency is too high for | `behavior_diff_gate.enabled: false` (or `harvest: false` / `block: false` — the two are separable) | `ORCHESTRAY_BEHAVIOR_DIFF_DISABLED=1`; harvest off: `ORCHESTRAY_FIXTURE_HARVEST=0` | default-on |
