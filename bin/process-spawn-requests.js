 ...
 * consume it on its next turn. Uses atomicAppendJsonl for safe concurrent
 * append (B5: shared lock with drainer not needed here since atomicAppendJsonl
 * is atomic for single-line rows; the drainer re-reads inside its own lock).
 */
...
function run() {
  ...
  // B2: serialize the entire read-filter-rewrite drain under a shared advisory
  // lock. Without this, parallel PreToolUse:Agent firings (parallel PM spawns)
  // race: each reads the same snapshot, both approve the same requests, both
  // rewrite spawn-requests.jsonl overwriting each other's changes. The lock is
  // keyed to spawn-requests.jsonl and shared with the housekeeper enqueuer
  // (B4) so every writer on this file contends the same mutex.
  const requestsPath = path.join(cwd, '.orchestray', 'state', 'spawn-requests.jsonl');
  const requestsLockPath = requestsPath + '.lock';

  _withAdvisoryLock(requestsLockPath, () => {
    // --- Read pending requests (inside the lock) ---
    ...
    if (pending.length === 0) {
      return; // nothing to do; safeExit0() called after the lock block
    }
    ...
    // --- Persist updated request list (inside the lock) ---
    if (changed) {
      writeSpawnRequests(cwd, entries);
    }
  });
