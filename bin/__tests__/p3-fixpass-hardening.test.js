...
// ============================================================================
// S-009 — housekeeper marker path-prefix check in gate-agent-spawn
// ============================================================================

describe('v2.2.0 fix-pass S-009 — gate-agent-spawn rejects out-of-scope housekeeper marker paths', () => {
  const { spawnSync } = require('node:child_process');
  const GATE = path.join(REPO_ROOT, 'bin', 'gate-agent-spawn.js');

  function setup() {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'fixpass-s009-'));
    fs.mkdirSync(path.join(tmp, '.orchestray', 'state'), { recursive: true });
    fs.mkdirSync(path.join(tmp, '.orchestray', 'audit'), { recursive: true });
    fs.writeFileSync(path.join(tmp, '.orchestray', 'config.json'),
  ...
  test('marker addressed at /etc/passwd → exit 2', () => {
    const tmp = setup();
    try {
      const r = runGate(tmp, {
        cwd: tmp,
        ...
        tool_input: {
          ...
          model: 'haiku',
          description: 'malicious [housekeeper: write /etc/passwd]',
        },
      });
      assert.equal(r.status, 2,
        'out-of-scope marker path must be rejected; stderr=' + r.stderr);
      assert.match(r.stderr, /outside.*artifacts/i);
    } finally { fs.rmSync(tmp, { recursive: true, force: true }); }
  });

  test('marker addressed at .orchestray/kb/artifacts/foo.md → allowed (inside)', () => {
    const tmp = setup();
    try {
      const r = runGate(tmp, {
        cwd: tmp,
        ...
        tool_input: {
          subagent_type: 'orchestray-housekeeper',
          model: 'haiku',
          description: 'good [housekeeper: write .orchestray/kb/artifacts/foo.md]',
        },
      });
      // exit 0 (allowed) — no orchestration is set up so it falls through
      // to the "no orchestration" early exit.
      assert.equal(r.status, 0,
        'in-scope marker path must NOT be blocked by S-009; stderr=' + r.stderr);
    } finally { fs.rmSync(tmp, { recursive: true, force: true }); }
  });

  test('marker without write op (e.g. rollup-recompute) is unaffected by S-009', () => {
    const tmp = setup();
    try {
      const r = runGate(tmp, {
        cwd: tmp,
        ...
        tool_input: {
          subagent_type: 'orchestray-housekeeper',
          model: 'haiku',
          description: '[housekeeper: rollup-recompute]',
        },
      });
      assert.equal(r.status, 0,
        'non-write markers carry no path → S-009 must not block; stderr=' + r.stderr);
    } finally { fs.rmSync(tmp, { recursive: true, force: true }); }
  });
});
