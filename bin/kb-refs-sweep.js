...
/** Regex for @orchestray:kb://<slug> */
const KB_REF_RE = /@orchestray:kb:\/\/([A-Za-z0-9_-]+)/g;

/** Regex for @orchestray:pattern://<slug> */
const PATTERN_REF_RE = /@orchestray:pattern:\/\/([A-Za-z0-9_-]+)/g;

/**
 * Conservative bare-slug context regex.
 * Matches: "See also: foo-bar", "Ref: baz-qux", "refers to foo-bar", etc.
 * Capture group 1 = the slug candidate.
 * We only flag when the slug has at least 4 characters and looks kebab-like.
 */
const BARE_SLUG_RE = /\b(?:see also|ref|refers? to|linked)[:\s]+([a-z][a-z0-9-]{3,40})\b/gi;
 ...
 */
function _scanFile(filePath, kbSlugs, patSlugs, projectRoot) {
  const findings = [];
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // --- @orchestray:kb://<slug> ---
    KB_REF_RE.lastIndex = 0;
    let m;
    while ((m = KB_REF_RE.exec(line)) !== null) {
      const slug = m[1];
      if (!kbSlugs.has(slug)) {
        findings.push({
    ...
    while ((m = PATTERN_REF_RE.exec(line)) !== null) {
      const slug = m[1];
    ...
    // --- Bare-slug conservative scan ---
    BARE_SLUG_RE.lastIndex = 0;
    while ((m = BARE_SLUG_RE.exec(line)) !== null) {
      const slug = m[1];
      // Only flag if slug not in either KB or patterns.
      if (!kbSlugs.has(slug) && !patSlugs.has(slug)) {
        findings.push({
          source_file: filePath,
          line: lineNo,
          matched_text: m[0],
...
async function runKbRefsSweep(options = {}) {
  ...
  for (const file of allFiles) {
    const { findings } = _scanFile(file, kbSlugs, patSlugs, cwd);
