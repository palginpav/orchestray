...
| `feature_demand_gate.enabled` | boolean | `true` | Enable shadow-mode quarantine tracking. When true, Orchestray observes which optional protocols run and logs candidates that would qualify for quarantine. |
| `feature_demand_gate.quarantine_candidates` | string[] | `[]` | Slugs to quarantine immediately without waiting for the 14-day observation window. Recognized slugs in v2.1.14: `pattern_extraction`, `archetype_cache`. |
| `handoff_body_cap.enabled` | boolean | `true` | Enforce the 2,000-token handoff artifact body cap. When enabled, the T15 validator warns above 2,500 tokens and blocks above 5,000 tokens when no `detail_artifact` pointer is present. |
| `handoff_body_cap.hard_block` | boolean | `false` | When true, the validator blocks (not just warns) at the 2,500-token soft threshold. Flip to `true` in v2.1.15 after migration data confirms agents use `detail_artifact` links. |
| `event_schema_shadow.enabled` | boolean | `true` | Use the 3.5 KB event-type shadow index instead of loading the full 150 KB schema file. Auto-disables after 3 shadow misses in 24 hours; re-enables on next shadow regen. |
...
| `tokenwright.l1_compression_enabled` | boolean | `false` | Enable L1 MinHash intra-prompt dedup (tokenwright). Default `false` — audit v2.2.20 found 0/477 production prompts matched any dedup-eligible heading. Canonical default defined in `bin/_lib/config-defaults.js`. Kill switch: set to `true` only after a heading-list audit confirms eligible prompts exist. |
