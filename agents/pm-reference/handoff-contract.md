...
## 9. MCP projection conventions

*(This section was added by R-PFX in v2.1.14; see event-schemas.md for the full
MCP field-projection reference.)*

When a Structured Result includes MCP tool output fields, project only the fields
that the next-hop agent needs. Omit raw tool payloads from `summary` and
`assumptions`; link them via `detail_artifact` if they exceed the body cap (§10).
