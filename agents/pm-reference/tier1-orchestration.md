...
> *[Rare path: drift-sentinel invariant-extraction (§4.D). PM loads tier1-orchestration-rare.md via Tier-2 dispatch when `enable_drift_sentinel` is true in `.orchestray/config.json`.]*

---

> *[Rare path: reasoning-trace distillation (§4.Y). PM loads tier1-orchestration-rare.md via Tier-2 dispatch when `enable_introspection` is true in `.orchestray/config.json`.]*

> *[Rare path: confidence-protocol injection (§3.Z), adaptive-verbosity budgeting (§3.Y), confidence-signal reading (§4.Z), and visual-review integration (§4.V). PM loads tier1-orchestration-rare.md via Tier-2 dispatch when `enable_backpressure` or `enable_visual_review` is true in `.orchestray/config.json`.]*

---

> *[Rare path: sentinel-check (§6.S, pause/cancel detection). PM loads tier1-orchestration-rare.md via Tier-2 dispatch when orchestration status is `paused` or when `.orchestray/state/cancel.sentinel` exists.]*

> *[Rare path: preview and redo (§6.T). PM loads tier1-orchestration-rare.md via Tier-2 dispatch when `.orchestray/state/redo.pending` exists or the invocation prompt contains "PREVIEW MODE".]*

> *[Rare path: PreCompact resilience block (§6.R). PM loads tier1-orchestration-rare.md via Tier-2 dispatch when a cost-budget-check hook has emitted a hard-block event in the current turn.]*
...
> *[Rare path: resilience dossier field reference (§7.R). PM loads tier1-orchestration-rare.md via Tier-2 dispatch when recovering from a resume/replay (evidenced by `.orchestray/state/orchestration.md` status field in {paused, redo_pending, replay_active}).]*
...
> *[Rare path: consequence-forecasting (§39) and drift-check (§39.D) protocols. PM loads tier1-orchestration-rare.md via Tier-2 dispatch when `enable_consequence_forecast` or `enable_drift_sentinel` is `true` in `.orchestray/config.json`.]*
