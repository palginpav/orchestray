...
1. Call `mcp__orchestray__history_query_events` with
   `{ orchestration_ids: [<completed orchestration_id>], event_types: ["auto_extract_staged"],
   limit: 1 }` and take the most-recent returned event. Do NOT `Read`
   `.orchestray/audit/events.jsonl` directly — it is a shared, growing file and this tool
   exists specifically to replace that pattern.
