...
/**
 * v2.0.20 hotfix regression — plugin statusline wiring.
 *
 ...
 * `subagentStatusLine` are honored). v2.0.20 reshapes it to `subagentStatusLine`.
 *
 * This test pins:
 ...
 *   3. `package.json.version` === `.claude-plugin/plugin.json.version` === `2.0.20`.
 *
...
describe('v2.0.20 — plugin settings.json statusline wiring', () => {
...
describe('v2.0.20 — version parity across package.json and plugin.json', () => {
  const pkg    = readJson('package.json');
  const plugin = readJson('.claude-plugin/plugin.json');

  test('package.json version is 2.0.20', () => {
    assert.equal(pkg.version, '2.0.20');
  });

  test('.claude-plugin/plugin.json version is 2.0.20', () => {
    assert.equal(plugin.version, '2.0.20');
  });
