...
- **`pm-router` Haiku entry-point gateway** routes trivial `/orchestray:run` tasks (typos, single-file edits, isolated questions) to a cheap Haiku-tier `pm-router` agent that solo-handles them end-to-end. Complex tasks escalate to the Opus PM at depth 1 with full orchestration delegation toolkit intact (v2.2.3 had a topology regression: escalates were spawned at depth 2 with no Agent tool and forced to solo; v2.2.4 fixes this). Other slash commands (`/orchestray:resume`, `/orchestray:redo`, `/orchestray:issue`, `/orchestray:review-pr`) bypass the router. Default on; `pm_router.enabled: false` or `ORCHESTRAY_DISABLE_PM_ROUTER=1` to opt out.
...
| **PM Router (v2.2.3+)** | Haiku-tier entry-point classifier for `/orchestray:run` (v2.2.3); in v2.2.4+ the routing decision is made directly by the slash command at depth 0. Reads the user task, decides solo (route to pm-router for Haiku end-to-end), escalate (spawn the Opus PM at depth 1 with full orchestration toolkit), or decline (refuse with reason). Default-fallback-to-escalate on any uncertain input. Other slash commands (`/orchestray:resume`, `/orchestray:redo`, `/orchestray:issue`, `/orchestray:review-pr`) bypass the router. Disable with `pm_router.enabled: false` or `ORCHESTRAY_DISABLE_PM_ROUTER=1`. |
